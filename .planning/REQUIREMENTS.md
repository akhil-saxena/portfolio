...
**Amended 2026-08-29, after three proof gaps this audit named were closed** (PUB-13's lightbox,
SEO-01's unfetched `og:image`, QUAL-03's absent measurement). The roll-up below is **49 Complete /
7 Partial / 20 Pending**: QUAL-03 moved Pending → Partial by acquiring a gate that answers it.
PUB-13 and SEO-01 were already Complete and had their evidence replaced rather than their verdict.
`gate:content` now chains **twelve** gates, not eleven.
...
| PUB-13 | Phase 5. Public Site | Complete | Suppression confirmed **by computed style in a browser, not by a grep**: `05-AUDIT.md` §2 ran the whole six-class suite twice, and `loadY` is 0 of 48 under `reduce`. `test/public/home.node.test.ts` asserts every snap declaration sits inside `prefers-reduced-motion: no-preference`; `test/public/shell.unit.test.ts` asserts scroll-behaviour is inside that query and never the reverse. `src/styles/work.css` carries a load-bearing `reduce` reset at (0,4,0) for the card hover, because the design system declared that transition outside any motion query (D-14). **The lightbox half is no longer a grep.** `05-AUDIT.md` §2a: the overlay is opened in Chromium at 390 × 844 in both motion projects and `getComputedStyle` read on the real element — `animationName` is `lightboxFade` in **both** (so there is an animation to suppress, which a duration-only check could not tell apart from an element that never animated), and `animationDuration` is **0.2s under `no-preference` and 1e-05s (0.01ms) under `reduce`**. The `no-preference` read is the discriminating control: the same instrument on the same element returns a real 200ms. Proven able to fail by **deleting the design system's guard line and rebuilding** — `[reduce]` redded naming the element at 0.2s while `[normal]` stayed green — and by two vacuity roots (no tiles; tiles with every `<script>` stripped), both of which fail rather than skip. `primitives.css` restored from outside the repository, `cmp` clean, and the rebuilt artefact byte-identical. |
...
| SEO-01 | Phase 5. Public Site | Complete | `test/public/seo.node.test.ts` audits **every** public HTML document in `dist/client` and asserts it audited no fewer: the full tag set on each, each non-empty; canonical and `og:url` absolute, on the site origin and **identical to each other**; `twitter:card` `summary_large_image` everywhere; every `og:image` absolute on the **image** origin (the plan had this wrong); each photo detail page carrying its own photograph at the large variant. Proven by fetching: **every canonical resolves 200 and lands on the page that declares it**, and every sitemap URL is exactly the canonical of the page it names — 51/51, both directions. **And the `og:image` is now fetched too, which nothing did before 2026-08-29.** Every one of those assertions was about the STRING; a card pointing at a renamed or lost object was a green build and a blank preview. `npm run gate:og-live` (`scripts/assert-og-images-live.mjs`) takes its targets from the ARTEFACT — 52 documents → 51 tags → **40 distinct URLs** — and **GETs** each, asserting 200 with an `image/*` content-type and a non-empty body. GET, not HEAD, deliberately and oppositely to `gate:liveness`: the question is what a *crawler* gets, and a crawler issues a GET; the two methods measurably disagree on this zone. It runs as its own CI step and on the ship path, and NOT in `gate:content`, because a network call inside `npm run build` would red a build whose code is fine. Proven able to fail: a dead URL (404, named with its document), a foreign origin (refused *before* any request), and `images.akhilsaxena.com/robots.txt` — a **live 200 that is not an image**, which a status-only check would have passed — plus three vacuity refusals, including a document carrying `og:image:alt` and the words `og:image` in prose, which yields **zero** targets and refuses. 15 self-test canaries per invocation. |
...
| QUAL-03 | Phase 8. Harden & Cut Over | **Partial** | **Now measured, by a gate that answers this question rather than a neighbouring one.** `npm run gate:app-css` (`scripts/assert-app-css-confined-to-layout.mjs`, in `gate:content`, so it runs on every build and twice in CI) judges **510 declarations across 30 files** — `src/styles/*.css`, every `<style>` block in `src/**/*.astro`, and every inline `style` attribute, template literal and JSX style object. **Operational definition:** application CSS may *arrange* (a property that positions, sizes, flows, fragments or scrolls a box) or *hand a design-system token through* (`var(--…)`, `inherit`, `currentColor`); it may never **originate** a value in a dimension the design system already names — colour, type, radius or motion. The property partition is an **allow-list end to end** (132 layout · 49 tokenised · 23 free, each free entry carrying a written reason), so a property nobody classified is a loud refusal rather than a silent hole. **Spacing is excluded, by argument, not by convenience:** DS-11 records that brand themes own colour, type and geometry but not spacing, the density axis has no mechanism until DS-10 lands, and `gate:ladder` already owns the gutter — and the exclusion saves nothing today, because every custom property `src/styles/` declares is already composed from `var(--space-*)`. **It found four real violations nothing else had seen** — `home.css` carried `font-weight: 400` and `500` ×3, literal copies of `--weight-regular` and `--weight-medium`, falsifying `05-DS-FINDINGS.md`'s *"no local copy of a design-system token value"*. Fixed by re-pointing at the tokens, and the fix is proven to have moved nothing **twice**: the computed `font-weight`, `font-family` and `font-size` of all five elements read in Chromium before and after are **identical**, and re-running the whole six-class audit left all six `05-X-home-state-*` captures **byte-identical on disk**. *(Two `05-X-work-*` captures did differ, by 1 and 5 bytes, on a page none of the changed declarations reaches — PNG-encoder nondeterminism, restored from HEAD and recorded here so nobody later reads capture bytes as a regression signal.)* **Proven able to fail:** eleven plants, each redding and naming its file and line — a hex colour, a *named* colour, a colour smuggled into a var() fallback, one laundered through a custom property, one inside a JSX style object, one inside the registered opaque style source, a literal font stack, a literal radius, an unclassified property, an unreadable `style={IDENT}`, and a **stale debt entry**; plus four vacuity refusals (missing root, no files, empty file, empty argument). `src/` restored and verified `diff -r` clean with an identical tree digest. 21 canaries and 19 anti-canaries run on every invocation. **Why Partial and not Complete:** the gate ships **four registered debts — declarations that violate the definition and still ship** — printed on every PASS, each stale entry a failure: `border-radius: 10px` ×3 (upstream finding D-20, no rung between 8 and 12), the card `transition`'s literal `0.25s`/`0.15s` (belongs to D-14; the block is deleted, not re-pointed, when that lands), and the `/admin` doorway's `font-family: system-ui, sans-serif` + `line-height: 1.5` (that page deliberately imports nothing from the design system, so a token would resolve to nothing; Phase 7 deletes the block). The row is not ticked while four known violations ship. **What the gate still cannot see** is recorded in its header: the computed cascade, a runtime-assembled value, and `node_modules`. |
...
| Phase 8 · Harden & Cut Over | 0 | 2 | 4 | 6 |
| **Total** | **49** | **7** | **20** | **76** |

**The seven Partials are the ones worth reading before Phase 8's go/no-go**, because each is a
row that looks done from a distance. *(QUAL-03 joined the list on 2026-08-29 by being MEASURED for
the first time — it moved Pending → Partial, which is a row getting stronger, not weaker: it now has
a gate that answers it and four named violations it refuses to round up.)*
...
7. **QUAL-03** — measured at last by `gate:app-css`, over 510 declarations, with an operational
   definition and eleven plants behind it. Four registered debts still ship: a `10px` radius the
   design system has no rung for (D-20), the card transition's literal timings (D-14), and the
   `/admin` doorway's two typographic literals (deleted by Phase 7).
