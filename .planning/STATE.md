...
| 04 photo pipeline | **10 / 10 COMPLETE — all five criteria met 2026-08-28** | A photograph goes from a local file to live CDN objects and a commit on `main` by command line alone, with no admin UI in existence. Two live runs: `33148622707` published `wildlife-gentlegiants`, `33158490277` re-published it with different bytes. **Criterion 5 proven by construction** — each URL's embedded hash is the sha256 prefix of the bytes it serves (`ff17a846…` old, `1de8c65e…` new), so the old object is orphaned and intact rather than overwritten, and a purge is impossible to need. **OD-4 held on live data:** `order` 40 and `categoryOrder` 6 unchanged across the replace, so a retry is a repair and not a renumbering. Suite **1007/1007 across 27 files**; CI and Deploy both green. |
