...
| 04 photo pipeline | **10 / 10 executed** | **The pipeline is real.** A photograph went from a local file to a live CDN object and a commit on `main` by command line alone, with no admin UI in existence — run `33148622707`, commit `e43ad79`, `photo: publish wildlife-gentlegiants`, authored by the App token. `astro sync` 39 → 40. **The three EXIF checks confirmed on real data:** `iso: 100` (the file carries `ISOSpeedRatings` with `ISO` undefined, so the mapper did not regress), `date: 2026-01-24` (capture, not ingestion), `dimensions: 3361×2241` (source intrinsic — OD-11's first genuine divergence). Each variant carries **its own** content hash, verified as the sha256 of the bytes actually served, and ships `max-age=31536000, immutable` against the zone's 14400. **Criterion 5 is NOT met** and is recorded as such: the round trip needs a second dispatch that was not run. Suite **1007/1007 across 27 files**. |
...
- **A test fixture that inherits the ambient checkout's refs cannot pass in both jobs.**
  `deploy.yml` pins `ref: <head_sha>` — a **detached HEAD with no local branch** — while CI checks out
  a branch. `partial-failure`'s sandbox did `git clone --bare REPO_ROOT origin`, which copies whatever
  refs happen to exist, so `refs/heads/main` was simply absent under Deploy and **ten integration cases
  failed for one missing ref**. The same commits were green in CI and red in Deploy from `5f8a451`
  onward, and I misread that as flakiness and re-ran it. **The tell I missed: the re-run failed with
  ten cases where the first failed with two — a deterministic environmental difference escalates, a
  race does not.** Fixed by having the fixture construct `BRANCH` itself. **Standing rule: a fixture
  constructs its own refs; it never reads the checkout it happens to be running inside.**
