...
| 04 photo pipeline | **7 / 10 executed** | **Waves 1-2 complete.** Wave 1: the 15 count assertions re-scoped per assertion, `src/lib/photo-pipeline.ts` as the one contract, and the hole where a manifest could lie about the bucket closed. Wave 2: toolchain pinned (`sharp` direct, `exif-reader` over the stale `exifr`), the record producer with an upsert that provably never renumbers `order`, `publishManifest` committing to `main` with bounded re-derive-and-retry, and the dispatch workflow with Akhil's placeholder-`alt` refusal wired in. Suite **873/873 across 22 files**; seven gates and `build` all exit 0. **Wave 3 (04-07) next; OD-9 and OD-10 still open.** |
