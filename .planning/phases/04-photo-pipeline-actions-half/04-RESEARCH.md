...
> | **OD-9** | **Option A — the new pipeline stops writing `private/*-clean.webp` entirely.** Watermarked variants only; the source files live on Akhil's disk and the bucket was never a backup. Enforced by **absence of a `private/` key path**, asserted at the emitted upload descriptors (`descriptors.length > 0` first). B and C were rejected because both need infrastructure that does not exist, so choosing either without provisioning first would add one new publicly-downloadable master per photo to a **live** exposure. **The existing 39 stay reachable** — that cleanup is deferred to Phase 8 and remains on its blocking-prerequisite line. |
> | **OD-10** | **Option B — `DateTimeOriginal ?? ingestionDate`.** Read the capture date from the uploaded source's EXIF, fall back to the ingestion date when absent. **Both branches must be asserted** using synthetic parsed EXIF objects, not regenerated fixtures. **Verify the tag name in `exif-reader` specifically** — `exifr` and `exif-reader` disagree on at least one tag (`0x8827` is `ISO` vs `ISOSpeedRatings`), and a silent rename here writes the ingestion date forever while looking correct. **Option C is IMPOSSIBLE, not merely costly:** it proposed backfilling the 39 from the R2 originals' EXIF, and wave 2 measured that the served variants *and* the `private/*-clean.webp` masters both carry **zero** EXIF — sharp strips by default and the camera sources were never committed. |
>
> **CONSEQUENCE OF OD-10 = B, carried to Phase 5 and recorded rather than discovered later.** The 39
> existing records mean *"the day it was published"*; every new record will mean *"the day it was
> taken"*. **This split is permanent** — the only reconciliation the research offered was the backfill,
> now known impossible, and the alternative is hand-entering 39 capture dates onto reviewed data.
> Phase 5 sorts and displays `date`, so it must decide whether to present it as a capture date, a
> publish date, or to stop showing it. Do not let a renderer silently imply one meaning for both.
