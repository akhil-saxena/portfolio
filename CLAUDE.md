...
<!-- ORIENTATION:start — hand-maintained, NOT generated by generate-claude-md -->
## Repository Orientation

**Pre-code state.** `main` is a from-scratch rebuild and is currently **pre-code**: there is
no `src/` and no `package.json` yet. Any instruction phrased as "modify `src/...`" or "run
`npm run ...`" is wrong until the rebuild creates those files — treat it as a planning
error, not a missing file. Run `ls -A` at the repo root to confirm what actually exists
before assuming a path is there.

**Where the legacy code is.** The legacy Next.js app is not checked out in this working
tree; it lives on the `legacy/nextjs-portfolio` branch. To read it without leaving `main`:

```bash
git show legacy/nextjs-portfolio:src/lib/access.ts   # read one legacy file
git ls-tree -r --name-only legacy/nextjs-portfolio   # list every legacy file
git checkout legacy/nextjs-portfolio                 # check the whole branch out
```

**Scope of the sections below.** `Technology Stack`, `Conventions`, and `Architecture`
describe that legacy Next.js 15 / `@cloudflare/next-on-pages` app in the present tense.
They are a **porting reference** — the inventory of what the rebuild must replace — and
explicitly **not** a description of the current working tree. The `## Project` section and
its `### Constraints` above are the opposite: they describe the Astro + React islands
target and ARE current, so nothing in them is legacy.

**Maintenance note.** Those three legacy sections are GSD-managed regions sourced from
`.planning/codebase/*.md`. If `generate-claude-md` regenerates them, their `(LEGACY …)`
heading suffixes and blockquote lead-ins are overwritten and must be re-applied by hand.
This orientation block sits outside the GSD markers and survives regeneration.
<!-- ORIENTATION:end -->
