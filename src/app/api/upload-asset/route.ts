...
export async function POST(request: NextRequest) {
  ...
  // Asset upload functionality requires a direct CF Pages Function with R2 binding access,
  // which is unavailable when _worker.js is present.
  // TODO: Implement via presigned URLs or an alternative upload path.
  return NextResponse.json(
    {
      error:
        "Asset upload is temporarily unavailable. R2 bindings are not accessible from Next.js route handlers.",
    },
    { status: 503 }
  );
}
